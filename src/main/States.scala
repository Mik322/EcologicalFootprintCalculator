...
object States {
  case class FootPrintState(ecologicalFootPrint: Double, transportTrips: List[TransportTrip], waste: Option[Waste], electricity: Electricity, water: Option[Double], staticData: StaticData)
  ...
  def createStates(newProfile: NewProfile): States = {
    ...
    val footPrint = FootPrintState(0, List(), None, Electricity(newProfile.footPrintData.electricityPerMonth, List()), None, staticData)
