...
object States {
  case class FootPrintState(ecologicalFootPrint: Double, transportTrips: List[TransportTrip], cars: List[Car], waste: Option[Waste], electricity: Electricity, staticData: StaticData)
  ...
  def createStates(newProfile: NewProfile): States = {
    ...
    val footPrint = FootPrintState(0, List(), List(), None, Electricity(newProfile.footPrintData.electricityPerMonth, List()), staticData)
