...
object States {
  case class FootPrintState(ecologicalFootPrint: Double, transportTrips: List[TransportTrip], cars: List[Car], waste: Option[Waste], energySources: List[EnergySource], water: Option[Double], footPrintData: FootPrintData)
  ...
  def createStates(newProfile: NewProfile): States = {
    ...
    val footPrint = FootPrintState(0, List(), List(), None, List(), None, footPrintData)
